...
namespace TimeTrack {
    class Win32 {
        ...
        public static void CaptureScreen(string fileName,int handle) {
            int hdcSrc = User32.GetWindowDC(handle);
            int hdcDest = CreateCompatibleDC(hdcSrc);
            int hBitmap = CreateCompatibleBitmap(hdcSrc,GetDeviceCaps(hdcSrc,8),GetDeviceCaps(hdcSrc,10));
            SelectObject(hdcDest,hBitmap);
            BitBlt(hdcDest,0,0,GetDeviceCaps(hdcSrc,8),
        ...
        private static void SaveImageAs(int hBitmap,string fileName) {
            Bitmap image = new Bitmap(Image.FromHbitmap(new IntPtr(hBitmap)),
            Image.FromHbitmap(new IntPtr(hBitmap)).Width,
            Image.FromHbitmap(new IntPtr(hBitmap)).Height);
        ...
        public static void SendForgroundKeyDown(ScanCodeShort wScan,VirtualKeyShort wVk) {
            INPUT[] pInputs = new INPUT[1];
            pInputs[0].type = 1;
            pInputs[0].U.ki.wScan = wScan;
            pInputs[0].U.ki.wVk = wVk;
